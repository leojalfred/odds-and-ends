...
- **A row hands its character to script with `AddScope`**, which is the answer to "one template, many rows": `GetScriptedGui('x').Execute( GuiScope.SetRoot( GetPlayer.MakeScope ).AddScope( 'leo_oae_mbge_gov', Character.MakeScope ).End )`, received because the scripted GUI declares `saved_scopes = { leo_oae_mbge_gov }`. The checkbox's `checked` reads the same scripted GUI's `is_shown`. That is one scripted GUI evaluation per row per frame, the only per-frame script in the panel, and the reason not to reach for this pattern where a plain variable would do.
